...
Functions:
	...
	smart_mail( $to_name, $to_email, $subject, $body, [$from_name], [$from_email], [$use_html = true] )
	...
	sports_pagination( [$show_if_empty = true] )
		Displays pagination for the current wp_query.
	
	smart_list_terms( $post_ID, $taxonomy, $sep = ', ', $use_hyperlinks = true, $limit = null, $limit_text = "%s more" )
		Returns a string containing a separated list of terms for the given post.
	
	auto_archive_image( [$post_id = false], [$meta_key = false] )
	...
	smart_file_download( $file, [$filename = null], [$attachment = true] )
...
function smart_mail( $to_name, $to_email, $subject, $body, $from_name = false, $from_email = false, $use_html = true ) {
...
function vimeo_get_info_by_url( $url, $option_id = null ) {
	...
	Here is the result on a successful request, and also what will be cached. The ellipsis are added by me, you'll get full responses:
	{
	  ...
	  media:             "<img src=\"http://example.com/wp-content/uploads/2013/10/461023899_128020.jpg\" alt=\"Vimeo: PORTRAIT\">",
	  media_id:          437,
	}
	*/
...
function sports_pagination( $show_if_empty = true ) {
...
function smart_list_terms( $post_ID, $taxonomy, $sep = ', ', $use_hyperlinks = true, $limit = null, $limit_text = "%s more" ) {
...
function smart_file_download( $file, $filename = null, $attachment = true ) {
