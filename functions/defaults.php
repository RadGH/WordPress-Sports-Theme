...
function ld_custom_wp_footer() {
	?>
	&copy;<?php echo date('Y'); ?> <?php bloginfo('name'); ?> &ndash; Powered by <a href="http://radforest.com/" target="_blank">RadForest</a> &ndash; Built on
...
<?php
	...
	if ( is_singular() ) { $metadata['og:image'] = auto_archive_image( get_the_ID(), 'full' ); }
	if ( !$metadata['og:image'] ) $metadata['og:image'] = smart_media_size( (int) get_field( 'open_graph', 'options', false ), 'full' );
	if ( !$metadata['og:image'] ) $metadata['og:image'] = smart_media_size( (int) get_field( 'logo', 'options', false ), 'full' );
