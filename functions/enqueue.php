...
function ld_enqueue_script( $filename, $deps = array(), $version = null, $in_footer = false ) {
	...
	wp_enqueue_script( $key, ld_enqueue_url( $filename ), $deps, $version, $in_footer );
}
...
function ld_enqueue_style( $filename, $deps = array(), $version = null, $media_types = 'all' ) {
	...
	$key = 'ld-' . pathinfo($filename, PATHINFO_FILENAME);
	wp_enqueue_style( $key, ld_enqueue_url( $filename ), $deps, $version, $media_types );
}
...
function ld_enqueue_theme_scripts() {
	...
	ld_enqueue_style(  '/includes/libraries/swipebox/css/swipebox.min.css', array(), '1.0.0' );
	ld_enqueue_script(  '/includes/libraries/swipebox/js/jquery.swipebox.min.js', array( 'jquery' ), '1.4.1' );
...
function ld_enqueue_login_scripts() {
	ld_enqueue_script( '/includes/admin/login.js', array( 'jquery' ) );
	ld_enqueue_style(  '/includes/admin/login.css' );
}
